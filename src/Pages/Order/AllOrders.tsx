...
function AllOrders() {
  ...
  const [filters, setFilters] = useState({searchString:"", status:""});//local state
  ...
  const handleChange =(
    ...
    const tempVal = inputHelper(e, filters);
    setFilters(tempVal);
   }
   ...
   useEffect(() =>
   {
    ...
    const totalRecords = JSON.parse(data.totalRecords).TotalRecords;
